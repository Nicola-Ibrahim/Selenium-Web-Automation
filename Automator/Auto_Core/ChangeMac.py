...
def changeMacAddress(desire_mac_address = None, change_wifi:bool = ..., change_ethernet:bool = ...):
    ...
    # Check which interface we want to change
    if(change_wifi==False and change_ethernet==True):
        target_value = "Realtek PCIe"
        mac_name_regex = re.compile(r"(Ethernet)")

    elif(change_wifi==True and change_ethernet==False):
        target_value = "Wireless"
        mac_name_regex = re.compile(r"(Wi-Fi)")
    
    elif((change_wifi==False and change_ethernet==False) or change_wifi==True and change_ethernet==True):
        print("Please select only one interface to change its MAC address")
        return (None, None)
    ...
    if(current_mac_address is not []):
        while(True):
            ...
            for adapter in network_adapters:
                ...
                if adapter_index_find and target_value in adapter:
