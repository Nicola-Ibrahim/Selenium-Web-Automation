...
class MacChanger(ABC):
    ...
    def change_mac_address(self, desire_mac_address = None):
...
class WifiMacChanger(MacChanger):
    ...
    def get_current_mac_address(self) -> str:
        ...
        getmac_output = run("getmac /v", capture_output=True).stdout.decode(encoding='ISO-8859-1').split('\n')
...
def changeMacAddress(adapter_name:str = ..., adapter_type:str = ..., desire_mac_address = None):
    ...
    if(adapter_type == 'Ethernet'):
        # adapter_name = "Realtek PCIe"
        mac_name_regex = re.compile(r"(Ethernet)")

    elif(adapter_type == 'Wi-Fi'):
        # adapter_name = "Wireless"
        mac_name_regex = re.compile(r"(Wi-Fi)")
    ...
    if(current_mac_address is not []):
        while(True):
            ...
            for adapter in network_adapters:
                ...
                # If there is an index and the adapter has wireless in description we are going to disable and enable the adapter
                if adapter_index_find and adapter_name in adapter:
                    disable = run(["wmic", "path", "win32_networkadapter", "where", f"index={adapter_index_find.group(0)}", "call", "disable"],capture_output=True)
                    # If the return code is 0, it means that we successfully disabled the adapter
                    # if(disable.returncode == 0):
                    #     print(f"Disabled {adapter.lstrip()}")
